...
- **Phase 3 - Tier-A heroes + LOD promotion - done.** Foreground heroes run the CPU pose pipeline
  ...
  (`shaders/skin_tierA.vert`). Press **P** (or `--promote`) to re-render the same heroes through the
  Tier-B baked path - the LOD-promotion A/B.
...
- **Phase 6 - benchmarking & profiling - done.** The demo now *measures* marrow honestly across CPU
  ...
  **jobified** (**J**): a small thread pool fans it across cores (the runtime owns no threads), with a
...
Controls: **WASD / Q / E** fly, hold **right mouse** to look, **Shift** to sprint, **M** cycle the
model (procedural biped + every baked model in the assets folder - see **glTF assets** below), **P**
toggle the heroes between Tier-A and Tier-B (LOD promotion), **T** toggle the crowd tier (GPU-baked ↔
CPU-batch), **B** cycle the CPU-batch backend (scalar/SSE2/AVX2) - **1 / 2 / 3** jump straight to
scalar / SSE2 / AVX2, **J** toggle jobified CPU-batch (fan the palette-gen across the job pool ↔
single-threaded), **− / =** halve / double the live entity count, **R** reset the perf stats, **F1**
toggle the HUD, **F2** HUD detail, **Esc** to quit.

### Flags

| flag | effect |
|---|---|
| `--count N` | initial number of crowd instances (default 1600). Adjustable live with **− / =** (halve / double) in `[1, 16384]` - both tiers always draw the same count for an honest A/B, so the interactive count is bounded by the CPU-batch cap (16384); buffers are sized to that capacity once, so resizing never reallocates. Higher GPU-only counts (e.g. 65536) are covered by the headless `--bench` sweep |
| `--promote` | start with the heroes rendered via the Tier-B baked path (same as pressing **P**) |
| `--crowd-tier {gpu\|cpu}` | crowd animation tier: GPU-baked (default) or live CPU-batch; **T** toggles at runtime |
| `--frames N` | render N frames and exit (headless) |
| `--screenshot PATH` | write a PNG of the last frame (swapchain readback) - e.g. `--frames 30 --screenshot shot.png` |
| `--validate` | run the CPU parity checks + the scalar/SSE2/AVX2 batch benchmark + CPU-crowd layout parity, print a report, exit (no window) |
| `--validate-gpu` | the above **plus** the GPU-skin-vs-CPU-oracle SSBO readback (creates a hidden device) |
...
**Live HUD.** Press **F1** for an overlay with FPS, frame time, **CPU work ms** (sum of update +
palette + record + submit - GPU-backpressure waits are excluded), **GPU ms** (from timestamp
queries; `n/a` if the queue can't time-stamp), the **`palette gen` vs `upload`** split, instance /
triangle / bone counts, the active tier + backend, and a colored frame-time graph. On the CPU tier it
also shows **`ns/(inst·bone)`** - `palette gen` normalized by the bone-instances the batch kernel
covered, the scale-invariant headline throughput number that's directly comparable across backends and
entity counts (the same figure the `--validate` microbench prints). **F2** expands a per-scope CPU +
per-zone GPU breakdown. The window title carries a one-line summary even with the overlay off.

**Live knobs.** **− / =** halve / double the entity count and **1 / 2 / 3** (or **B**) switch the CPU
backend without leaving the loop - so you can watch `ns/(inst·bone)` and the frame-time graph respond
in real time. Each change clears the rolling stats automatically (and **R** clears them on demand) so
the percentiles settle on the new configuration instead of averaging across the transition.

**Two crowd tiers, A/B at the same scale.** The default **GPU-baked** crowd samples the baked palette
texture on the GPU (`palette gen` reads 0 - there is no CPU palette work). Press **T** (or
`--crowd-tier cpu`) to switch to the **CPU-batch** tier, which runs `mrw_batch_clip_to_palette` every
frame, grouped per clip into homogeneous batches, into 64-byte-aligned cacheable RAM (timed
`PALETTE_GEN`) and then `memcpy`s into the mapped SSBO (timed `PALETTE_UPLOAD`). **B** cycles
scalar → SSE2 → AVX2; watch `palette gen` move (AVX2 < scalar) while the upload stays flat. Both
tiers render the same grid at the same scale, so the only visible difference is the Tier-A/Tier-B
fidelity gap.
...
slice and uses its **own** scratch. Press **J** to toggle it; the backend label reads e.g. `AVX2 x8`
when jobified, and `palette gen` / `ns·(inst·bone)` drop by roughly the lane count. The
`demo_validate` gate proves it is **bit-for-bit identical** to the serial batch (per-instance results
...
The same flags work - e.g. `--validate`, `--promote`, `--frames 30 --screenshot ual1.png`. The
crowd's Tier-B path requires the rig to be Tier-B-eligible (UAL1 is); an ineligible rig bakes a
Tier-A-only `.mrw` and the crowd reports a missing `BAKED` section.
